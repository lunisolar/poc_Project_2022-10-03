...
class FairUpgradePolicy implements UpgradePolicy {
        ...
        for (int i = assignments.size() - 1; i >= 0; i--) {
            ...
            for (int k = i + 1; k < assignments.size() && !upperTier.isFull(); k++) {
                ...
                while (!upperTier.isFull() && lowerTier.isFull() && (pulled = lowerTier.pullForUpgrade()).isPresent()) {
                    ...
                    for (int middle = k - 1; middle > i; middle--) {
                        var tierMiddle = assignments.get(middle);
                        pulled = Opt.obj(tierMiddle.swapPull(pulled.get()));
                    }
